...
https://github.com/facebookresearch/SlowFast (Apache-2.0 license)
...
class I3DModel(nn.Module):
    def __init__(self, config):
        ...
        dim_inner = config["inner_channels"]

        self.stem = VideoModelStem(
            dim_in=config["input_channel"],
            dim_out=dim_inner,
            kernel=config["temp_kernels"][0] + [7, 7],
            stride=[1, 2, 2],
            padding=[config["temp_kernels"][0][0] // 2, 3, 3],
        )

        self.s2 = ResStage(
            dim_in=config["channels"][0],
            dim_out=config["channels"][1],
            stride=config["spatial_strides"][0],
            temp_kernel_sizes=config["temp_kernels"][1],
            num_blocks=config["num_blocks"][0],
            dim_inner=dim_inner,
            num_groups=1,
            num_block_temp_kernel=config["num_blocks_temporal_kernel"][0],
            nonlocal_inds=config["nonlocal"]["location"][0],
            nonlocal_group=None,
            nonlocal_pool=None,
            dilation=config["spatial_dilations"][0],
        )

        self.s3 = ResStage(
            dim_in=config["channels"][1],
            dim_out=config["channels"][2],
            stride=config["spatial_strides"][1],
            temp_kernel_sizes=config["temp_kernels"][2],
            num_blocks=config["num_blocks"][1],
            dim_inner=dim_inner * 2,
            num_groups=1,
            num_block_temp_kernel=config["num_blocks_temporal_kernel"][1],
            nonlocal_inds=config["nonlocal"]["location"][1],
            nonlocal_group=None,
            nonlocal_pool=None,
            dilation=config["spatial_dilations"][1],
        )

        self.s4 = ResStage(
            dim_in=config["channels"][2],
            dim_out=config["channels"][3],
            stride=config["spatial_strides"][2],
            temp_kernel_sizes=config["temp_kernels"][3],
            num_blocks=config["num_blocks"][2],
            dim_inner=dim_inner * 4,
            num_groups=1,
            num_block_temp_kernel=config["num_blocks_temporal_kernel"][2],
            nonlocal_inds=config["nonlocal"]["location"][2],
            nonlocal_group=None,
            nonlocal_pool=None,
            dilation=config["spatial_dilations"][2],
        )

        self.s5 = ResStage(
            dim_in=config["channels"][3],
            dim_out=config["channels"][4],
            stride=config["spatial_strides"][3],
            temp_kernel_sizes=config["temp_kernels"][4],
            num_blocks=config["num_blocks"][3],
            dim_inner=dim_inner * 8,
            num_groups=1,
            num_block_temp_kernel=config["num_blocks_temporal_kernel"][3],
            nonlocal_inds=config["nonlocal"]["location"][3],
            nonlocal_group=None,
            nonlocal_pool=None,
            dilation=config["spatial_dilations"][3],
        )

        self.head = ResNetBasicHead(
            dim_in=config["channels"][4],
        ...
        self.layers = nn.Sequential(
            self.stem, self.s2, self.s3, self.s4, self.s5, self.head
        )

        self.encoder = nn.Sequential(self.stem, self.s2, self.s3, self.s4, self.s5)
...
class ResStage(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            temp_kernel_sizes (int): temporal kernel size of the
                convolution in the bottleneck.
            stride (int): list of the p stride of the bottleneck.
        ...
        assert all(
            (num_block_temp_kernel <= num_blocks for i in range(len(temp_kernel_sizes)))
        )
        self.num_blocks = num_blocks
        self.nonlocal_group = nonlocal_group
        self._drop_connect_rate = drop_connect_rate
        self.temp_kernel_sizes = (temp_kernel_sizes * num_blocks)[
            :num_block_temp_kernel
...
class ResNetBasicStem(nn.Module):
    ...
    def __init__(
        ...
        print(kernel, stride, padding, dim_in, dim_out)
        self.kernel = kernel
        self.stride = stride
        self.padding = padding
        self.inplace_relu = inplace_relu
...
class ResNetBasicHead(nn.Module):
    ...
    def __init__(
        ...
        Args:
            dim_in (int): the list of channel dimensions of the p inputs to the
                ResNetHead.
